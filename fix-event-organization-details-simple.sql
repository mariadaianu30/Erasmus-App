-- Fix organization details for existing events (SIMPLIFIED VERSION)
...
-- Step 5: Test the trigger with a sample event (SIMPLIFIED)
DO $$
DECLARE
    test_user_id UUID;
    test_event_id UUID;
    test_org_name TEXT;
    test_org_website TEXT;
BEGIN
    ...
    IF test_user_id IS NOT NULL THEN
        ...
        BEGIN
            ...
            INTO test_org_name, test_org_website
            FROM events 
            WHERE id = test_event_id;
            
            RAISE NOTICE 'SUCCESS: Trigger test passed. Organization name: %, Website: %', test_org_name, test_org_website;
